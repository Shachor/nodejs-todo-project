...
describe('POST /todos', () => {
  ...
  it('should create a new todo', (done) => {
    ...
    request(app)
      .post('/todos')
      .set('x-auth', users[0].tokens[0].token)  // We have turned on authentication, this is necessary to 'LOGIN'
...
describe('GET /todos/:id', () => {
  ...
  it('should not return todo doc created by another user', (done) => {
    request(app)
      ...
      //declare the ObjectID in the todo declaration above. Here we need to
      //convert it to a string using .toHexString so we can pass it to request
      .get(`/todos/${todos[1]._id.toHexString()}`)    // We're sending user 2 id with user 1 token
      .set('x-auth', users[0].tokens[0].token)
      .expect(404)
      .end(done);
  });
...
describe('DELETE /todos/:id', () => {
  it('should delete todo by id', (done) => {
    ...
    request(app)
      ...
      .end((err, res) => {
        ...
        //query db with findById and make sure it doesn't exist (toBeFalsy)
  ...
  it('should not delete todo that is not mine', (done) => {
    var hexId = todos[1]._id.toHexString();     // user 1...
    request(app)
      .delete(`/todos/${hexId}`)
      .set('x-auth', users[0].tokens[0].token)  // tries to delete user 2 data
      .expect(404)
      // .end(done());
      .end((err, res) => {
        if (err) {
        ...
        //query db with findById and make sure it STILL exists (toBeTruthy)
...
describe('PATCH /todos/:id', () => {
  ...
  it('should not update the todo if not mine', (done) => {
    //grab id of first item
    var hexId = todos[1]._id.toHexString();
    var updatedTodo = {text: 'I am soooo tired', completed: true};
    //make patch request
  ...
  it('should clear completedAt when todo is not completed', (done) => {
    ...
    //update text, set compelted to false
      //assert 200
      //assert response.body is now text and completed is now false, completedAt is null .toBeFalsy
  });
});
// ==========================================================================
...
describe('POST /users', () => {
   it('should create a user', (done) => {
      ...
      request(app)
         ...
         .expect(200)
         .expect((res) => {
            expect(res.headers['x-auth']).toBeTruthy();     // Use [] because x-auth has hypen in it
